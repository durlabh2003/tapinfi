  ...
  useEffect(() => {
    async function fetchBlogs() {
      ...
      try {
        ...
        const parsedBlogs = (data || []).map((b: any) => {
          let sections: BlogSection[] = [];
          if (Array.isArray(b.sections)) {
            sections = b.sections;
          } else if (typeof b.sections === 'string') {
            try {
              sections = JSON.parse(b.sections);
            } catch {
              sections = [];
            }
          }
      ...
      <main className="pt-[110px]">
        ...
        <div className="px-4 sm:px-8 lg:px-20 py-12 text-center">
          ...
          <ScrollReveal animation="fade-up" delay={100}>
            ...
            >
              Insights, tips, and stories about the future of digital networking &amp; smart business cards
            </p>
          </ScrollReveal>
        </div>
                  ...
                  <ScrollReveal key={post.id} animation="fade-up" delay={(i % 3) * 80}>
                    <Link
                      ...
                      className="group flex flex-col rounded-2xl overflow-hidden border border-gray-200
                        ...
                        hover:border-[#5aa4f4]
                        hover:-translate-y-2
                        hover:shadow-[0_16px_40px_rgba(90,164,244,0.15)] bg-white h-full"
                    >
                      {/* Card Primary Cover Photo */}
                      <div className="h-[220px] relative overflow-hidden bg-gray-100">
                        ...
                        {/* Section Count Badge */}
                        {sectionCount > 0 && (
                          <div className="absolute top-4 right-4 bg-black/70 backdrop-blur-md text-white text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider border border-white/20">
                      ...
                      <div className="p-5 flex flex-col flex-1 bg-white">
                        ...
                        <span
                          className="text-[#5aa4f4] text-[13px] font-semibold group-hover:underline
                            flex items-center gap-1 transition-all duration-200 mt-auto"
                          style={{ fontFamily: "'Poppins', sans-serif" }}
                        >
                          Read Full Article
